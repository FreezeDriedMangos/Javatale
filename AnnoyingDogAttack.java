...
public class AnnoyingDogAttack extends Attack{
    ...
    private static final Projectile FRISBEE = new Projectile("imgs/frisbee_big.png", 0, 0, new Vector(3, 0), 11);
    private static final Projectile DOG = new Projectile("imgs/dogProjectile.png", 0, 0, new Vector(2.5, 0), 7);
    private static final Projectile SOLID_BOX = new Projectile("imgs/solidBox.png", 0, 0, new Vector(2.5, Math.PI /2), 7);
    private static final Projectile BREAKABLE_BOX = new Projectile("imgs/breakableBox.png", 0, 0, new Vector(2.5, Math.PI /2), 7);
    private static final Projectile BONE_1 = new Projectile("imgs/bones/bone1.png", 0, 0, new Vector(4.5, 0), 7); //3.2
    
    private int frisbeeSpeed = 3;
    private int frisbeeDamage = 11;
    ...
    public AnnoyingDogAttack(int attack, BoxLabel box, JLayeredPane panel, Soul soul){
        ...
        if(attack == FRISBEE_WALL_1){
            ...
            projectileManagers.add(new TargetedPM(panel, Main.PROJECTILE_LAYER_VISIBLE, soul));
            projectileManagers.get(0).spawnProjectile(new Projectile("imgs/bark.png", 320 - 33, 100, new Vector(3.5, 0), 11));
        } else if (attack == GREEN){
            Projectile template = new Projectile("imgs/ball.png", 0, 0, new Vector(3, Math.PI), 6);
            GreenSoulPM pm = new GreenSoulPM(panel, "imgs/ball.png", new int[]{1,1,1,4,2,4,2,3,3,3}, 32, 6, 3, soul);
            projectileManagers.add(pm);
            ...
            int distanceBetweenRows = 200;
            int distanceBetweenDogs = box.getWidth() / 3;
            int x = box.getX();
            for(int i = 0; i < 5; i++){ //number of rows of dogs
                for(int j = 0; j < 3; j++){ //number of dogs in a row
                    pm.spawnProjectile(new Projectile("imgs/dogProjectileVertical.png", j*distanceBetweenDogs + x, -(distanceBetweenRows * i), new Vector(2.5, Math.PI /2), 7));
                }
            }
    ...
    public void tickUpdate(double delta, Soul soul){
        if(attack == AIMED_FRISBEES_HORIZONTAL){
            if(age % 20 == 0){
                projectileManagers.get(0).spawnProjectile(new Projectile("imgs/frisbee_big.png", box.getX() - 64, soul.getY(), new Vector(frisbeeSpeed, 0), frisbeeDamage));
            }
        } else if(attack == AIMED_FRISBEES_0){
            if(age % 80 == 0){
                projectileManagers.get(0).spawnProjectile(new Projectile("imgs/frisbee_big.png", box.getX() - 128, soul.getY(), new Vector(frisbeeSpeed, 0), 1));
            }
        } else if (attack == BARK_0){
            //formula:
            ...
            if((age - 20) % 120 == 0 || age % 120 == 0){
                projectileManagers.get(0).spawnProjectile(new Projectile("imgs/bark.png", 320 - 33, 100, new Vector(3.5, 0), 2));
                SoundPlayer.play("mods/AnnoyingDog/sounds/dogBark.wav");
            }
        } else if (attack == BARK_1){
            if(age == 20){
                projectileManagers.get(0).spawnProjectile(new Projectile("imgs/bark.png", monster.getX(), monster.getY(), new Vector(3.5, 0), 11));
                SoundPlayer.play("mods/AnnoyingDog/sounds/dogBark.wav");
            ...
            if(age == 370){
                ...
                int distanceToCenter = 400;
                int y = box.getY() + 5; 
                double speed = BONE_1.velocity.a();
                
                for(int i = 0; i < 28; i++){ //number of collumns
                    if(i % 2 == 0){
                        pm.spawnProjectile(new Projectile(BONE_1, centerX - distanceToCenter - BONE_1.getWidth(), y));
                    } else {
                        BONE_1.velocity.setA(-speed);
                        pm.spawnProjectile(new Projectile(BONE_1, centerX + distanceToCenter, y));
                        BONE_1.velocity.setA(speed);
                ...
                GreenSoulPM pm = new GreenSoulPM(panel, "imgs/ball.png", new int[]{1,1,1,4,2,4,2,3,3,3,4,3,4,2,1,3}, 32, 6, 3, soul);
                projectileManagers.add(pm);
            } else if (age == 370 * 3){
                    //make the projectiles from the last part invisible and non-damaging
                    projectileManagers.get(0).setNotCollidable();
            ...
            if(age == 370){
                ...
                for(int i = 0; i < 8; i++){ //number of collumns
                    if(i % 2 == 0){
                        boneHeight = (int)(Math.random()*9 + 0.5) + 4; //picks a random bone height between 4-13, inclusive
                        x = centerX - distanceToCenter - BONE_1.getWidth();
                        filepath = "imgs/bones/bone" + boneHeight + ".png";
                ...
                //set up the next portion of the attack (green)
                GreenSoulPM pm = new GreenSoulPM(panel, "imgs/ball.png", new int[]{2,1,2,3,2,4,2,1,4,2,3,2,1,3,1,4,2,1,3,1,3}, 32, 6, 3.3, soul); //{1,1,1,4,2,4,2,3,3,3,4,3,4,2,1,3}
                projectileManagers.add(pm);
            } else if (age == 370 * 3){
                    //make the projectiles from the last part invisible and non-damaging
                    projectileManagers.get(0).setNotCollidable();
                ...
                //spawn the extra boxes
                BREAKABLE_BOX.velocity = new Vector(3.5, Math.PI /2);
                y = 0;
                int randX = 0;
                int range = box.getWidth();
                for(int i = 0; i < 16; i++){
                    randX = (int)(Math.random() * range + 0.5);
                    pm.spawnProjectile(new Projectile(BREAKABLE_BOX, box.getX() + randX, y)); //spawn the breakable block
                    
                    y -= projectileWidth * 6;
                }
                
                BREAKABLE_BOX.velocity = new Vector(2.5, Math.PI /2);
                projectileManagers.add(pm);
            } else if (age == 370 * 5){
                //make the projectiles from the last part invisible and non-damaging
                projectileManagers.get(0).setNotCollidable();
        ...
        //this is atypical of an Attack subclass. I have it here just to do something special
        if(variation && age >= 370 * 6){
            if(age == 370 * 6){
                SoundPlayer.play("mods/AnnoyingDog/sounds/hotdog.wav");
            }
            ...
            direction.setAB(soul.getX() - x, soul.getY() - y);
            direction.setR(3);
                        
            if(!soul.area.intersects(x, y, width, height)){
                x += direction.a();
                y += direction.b();
                monster.setLocation(x, y);
            } else {
                soul.hp = 0;
                SoundPlayer.play("mods/AnnoyingDog/sounds/dogResidue.wav");
            }
        }
    }
}
